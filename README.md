...
| **VPN** | Cloud Headscale (GCP) + Watchdog | GCP 항시 가동, On-prem SPOF 해소, DaemonSet 연결 감시 |
...
| 컴포넌트 | 설명 |
|----------|------|
| **Backstage** | 개발자 포털, 서비스 카탈로그, 셀프서비스 프로비저닝, AI 챗봇 (Infra Assistant) |
...
### Crossplane Compositions (16개 XRD)

| 클라우드 | XRD | 리소스 타입 |
|----------|-----|------------|
| **GCP** | XGCPInstance, XBucket, XDatabase, XCluster, XCache, XPubSub, XWebApp, XClusterBurst | VM, Storage, Database, GKE, Cache, PubSub, WebApp, **Burst Cluster** |
| **AWS** | XEC2Instance, XS3Bucket, XRDSDatabase, XEKSCluster | EC2, S3, RDS, EKS |
| **Azure** | XAzureVM, XAzureBlobStorage, XAzureDatabase, XAKSCluster | VM, Blob Storage, PostgreSQL/MySQL, AKS |
...
### Backstage Infra Assistant (AI 챗봇)

Backstage에 내장된 AI 챗봇 플러그인으로, 개발자가 인프라 관련 질문을 하면 적합한 템플릿·스펙을 추천합니다.

- **백엔드**: Azure OpenAI (GPT-4.1-mini)
- **기능**: 클라우드 리소스 선택 가이드, 적합한 Scaffolder 템플릿 추천, 스펙 설정 안내
- **UI**: Backstage 우측 하단 플로팅 챗 위젯
...
### 웹 UI 인증 (OAuth2 Proxy + Dex SSO)

내부 서비스 UI에 Dex SSO 기반 인증을 적용하여 미인증 접근을 차단합니다.

| 서비스 | OAuth2 Proxy | 인증 방식 |
|--------|-------------|-----------|
| KubeCost | `kubecost-oauth-proxy` | Dex OIDC → GitHub 조직 멤버십 |
| Longhorn UI | `longhorn-oauth-proxy` | Dex OIDC → GitHub 조직 멤버십 |
| Grafana | 내장 OAuth | Dex OIDC 직접 연동 |
| ArgoCD | 내장 OIDC | Dex OIDC 직접 연동 |

### 인증서 관리 (Cert-Manager)

- **CA Issuer**: 클러스터 내부 서비스 간 TLS 인증서 자동 발급
- **Hubble 인증서**: Cilium Hubble UI용 TLS 인증서 관리
...
- 16종 ExternalSecret으로 자동 동기화 (argocd-oidc, backstage-backend, dex-github, grafana-oauth, loki/tempo-minio, longhorn-backup, tailscale-auth, kubecost-cloud 등)
...
### Grafana 대시보드 (Code as Dashboard)

모든 대시보드를 ConfigMap으로 관리하여 GitOps로 버전 관리합니다.

| 대시보드 | 용도 |
|----------|------|
| K8s Cluster Monitoring | 클러스터 전체 상태 올인원 (노드/파드/네트워크/API 서버) |
| Trip-App Scenario | 장애 분석 4단계 (Metrics → Logs → Traces → JVM 상관관계) |
| ArgoCD Overview | ArgoCD 앱 sync 상태, health, 배포 메트릭 |
| LGTM Stack Health | Loki/Grafana/Tempo/Alloy/Prometheus 내부 상태 모니터링 |
| Alertmanager | 알림 라우팅 상태, silence/inhibit 현황 |
| KubeCost Ops Summary | 비용 분석 및 최적화 요약 |
| Backup Pipeline | Longhorn/Velero/MinIO 백업 상태 통합 |
...
| 자동화 | 설명 |
|--------|------|
| 버킷 초기화 Job | 클러스터 배포 시 `velero`, `loki`, `longhorn-backups`, `tempo` 버킷 자동 생성 |
| S3 미러링 CronJob | 매일 04:30 UTC `mc mirror`로 MinIO → AWS S3 오프사이트 복제 |
...
| `trip-backend.yaml` | `trip-app/backend/**` push | Docker build/push → sync |
| `trip-front.yaml` | `trip-app/frontend/**` push | Docker build/push → sync |
| `cloud-credit-monitor.yaml` | `scripts/cloud-credit-monitor.py` push | Docker build/push |
| `lifecycle-scanner.yaml` | `lifecycle-scanner/**` push | Docker build/push |
...
| 5. AI 챗봇 지원 | Backstage Infra Assistant에 인프라 질문 | AI가 적합한 템플릿·스펙 추천, 프로비저닝 가이드 제공 |

**핵심 메시지:** 개발자는 클라우드 CLI/콘솔을 몰라도 Backstage UI만으로 리소스 프로비저닝이 가능하며, AI 챗봇(Infra Assistant)이 어떤 리소스를 어떤 스펙으로 만들어야 하는지 안내하고, 수명주기 정책(Kyverno)이 자동 적용됩니다.
